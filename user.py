...
def getbankMoney(_name, _row):
    print("user.py - getMoney")
    loadFile()
    print(_name, "의 은행 자본을 탐색")
    result = ws.cell(_row, c_bank).value
    print(_name, "의 보유 자산: ", result)
    saveFile()
    return result
...
def getref(_row):
    return ws.cell(_row, c_refill).value


def iinit(_author, _row):
    modifyMoney(_author, _row, 1000000)
    _temp = ws.cell(_row, c_refill).value - 1
    ws.cell(row=_row, column=c_refill, value=_temp)


def in_bank(_author, _row, _amount):
    if int(_amount) < 0:
        return False
    if int(_amount) > ws.cell(_row, c_money).value:
        return False
    modifyMoney(_author, _row, -int(_amount))
    __amount = ws.cell(_row, c_bank).value + int(_amount)
    ws.cell(row=_row, column=c_bank, value=int(__amount))
    return True


def out_bank(_author, _row, _amount):
    if int(_amount) < 0:
        return False
    if int(_amount) > ws.cell(_row, c_bank).value:
        return False
    modifyMoney(_author, _row, int(_amount))
    __amount = ws.cell(_row, c_bank).value - int(_amount)
    ws.cell(row=_row, column=c_bank, value=int(__amount))
...
def cashranking():
    ...
    for row in range(2, 2 + userNum):
        _name = ws.cell(row, c_name).value
        _money = ws.cell(row, c_money).value + ws.cell(row, c_bank).value
...
def Signup(_name, _id):
    ...
    ws.cell(row=_row, column=c_refill, value=3)
    print("초기 리필량 설정 | refill:", ws.cell(_row, c_refill).value)
    ws.cell(row=_row, column=c_bank, value=0)
    print("초기 손실 설정 | bank:", ws.cell(_row, c_bank).value)
    print("")
...
def userInfo(_row):
    ...
    _bank = ws.cell(_row, c_bank).value
    ...
    return _lvl, _exp, _money, _bank, _loss
...
def setrefill(_row, _amount):
    loadFile()
    ws.cell(_row, c_refill).value = _amount
    saveFile()


def setloss(_row, _amount):
    loadFile()
    ws.cell(_row, c_loss).value = _amount
    saveFile()
...
# def x_change(_row, _column, value)
